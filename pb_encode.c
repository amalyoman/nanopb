...
extern pb_callback_t * sobiFLASH_MODS_GetEncodeCallbackField(pb_size_t tag);
    ...
    const pb_field_t *field, const void *pData)
{
    ...
    callback =  sobiFLASH_MODS_GetEncodeCallbackField(field->tag);
    if (!callback)
        return false;
...
bool checkreturn pb_encode(pb_ostream_t *stream, const pb_field_t fields[], const void *src_struct)
{
    ...
    size_t prev_size = 0;
    const pb_field_t *field_f = fields;
    pb_field_t _field_ram;

    while (1)
    {
        field = &_field_ram;
        memcpy_P(&_field_ram,field_f,sizeof(*fields));
        field_f += 1;
        if (field->tag == 0)
            break;
