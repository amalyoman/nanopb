...
extern pb_callback_t * sobiFLASH_MODS_GetDecodeCallbackField(void *start,pb_size_t tag);

/* Iterator for pb_field_t list */
typedef struct {
    const pb_field_t *start; /* Start of the pb_field_t array */
    const pb_field_t *pos; /* Current position of the iterator */
    pb_field_t _field_ram; /* Filled when start is changed. The iterator points to this */
...
static void pb_field_init(pb_field_iterator_t *iter, const pb_field_t *fields, void *dest_struct)
{
    ...
    memcpy_P(&iter->_field_ram,fields,sizeof(pb_field_t));
    iter->pos = &iter->_field_ram;
    iter->field_index = 0;
...
static bool pb_field_next(pb_field_iterator_t *iter)
{
    ...
    iter->pos++;
    iter->field_index++;
    memcpy_P(&iter->_field_ram, &iter->start[iter->field_index] ,sizeof(pb_field_t));
    iter->pos = &iter->_field_ram;
    if (iter->pos->tag == 0)
    {
        iter->pos = iter->start;
...
static bool checkreturn decode_callback_field(pb_istream_t *stream, pb_wire_type_t wire_type, pb_field_iterator_t *iter)
{
    ...
    pCallback =  sobiFLASH_MODS_GetDecodeCallbackField((void *)iter->start,iter->pos->tag);
    if (!pCallback)
        return false;
    ...
    pb_extension_t *extension, uint32_t tag, pb_wire_type_t wire_type)
{
    ...
    iter.start = field;
    iter.pos = field;
    memcpy_P(&iter._field_ram,field,sizeof(pb_field_t));
    iter.pos = &iter._field_ram;
    iter.field_index = 0;
    iter.required_field_index = 0;
